...
# Load all the data
CERES_nc = Dataset('/scratch/leuven/336/vsc33653/gridding/CERES/CERES_grid0-2deg_SYN1deg-1H_Terra-Aqua-MODIS_Ed4.1_Subset_20171101-20210731_NH.nc')
...
#PARint = Dataset('/scratch/leuven/336/vsc33653/gridding/CERES/CERES_0-2deg_int_t1099_t1340_NH_SYN1deg_Terra-Aqua-MODIS_Ed4.1_Subset_20171101-20210731.nc',
#                    mode='w', format='NETCDF4')
PARint = Dataset('/scratch/leuven/336/vsc33653/gridding/CERES/CERES_0-2deg_int_t1099_t1340_NH_SYN1deg_Terra-Aqua-MODIS_Ed4.1_Subset_20171101-20210731.nc', mode='r+')
"""
...
for lat1 in range(len(lat_SIF)):
    for lon1 in range(len(lon_SIF)):
        if peat_mask[lat1, lon1]:
            PAR_pix = PARint.variables['PAR_int'][lat1, lon1, :]
            if not np.isnan(PAR_pix).all():
                PAR_ser = pd.Series(PAR_pix, index=time_SIFgreg)
                # PAR_ser = PAR_ser.resample('1D').mean()
