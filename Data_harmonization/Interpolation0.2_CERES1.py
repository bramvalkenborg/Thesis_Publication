...
PARint = Dataset('/scratch/leuven/336/vsc33653/gridding/CERES/CERES_0-2deg_int_t325_t425_NH_SYN1deg_Terra-Aqua-MODIS_Ed4.1_Subset_20171101-2021130.nc',
...
tCERES_x = [0, 0]
PAR_y = [0, 0]
# PAR_int = np.ma.masked_equal(np.zeros((len(lat_SIF), len(lon_SIF), len(time_SIF))), 0)
for t in range(325, 425):
            ...
            for lon in range(len(lon_SIF)):
                ...
                if peat_mask[lat, lon]:
                    ...
                    if cond[lat, lon]:
                        ...
                        # PAR_int[lat, lon, t] = PAR_y[0] + (tSIF - tCERES_x[0]).seconds / (tCERES_x[1] - tCERES_x[0]).seconds * (PAR_y[1] - PAR_y[0])
                        # PARint['PAR_int'][lat, lon, t] = PAR_int[lat, lon, t]
                        ...
                        # PAR_int[lat, lon, t] = np.ma.masked
                        # PARint['PAR_int'][lat, lon, t] = PAR_int[lat, lon, t]
